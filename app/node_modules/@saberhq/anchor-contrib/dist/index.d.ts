...
export declare type ContextAccounts<A extends IdlAccountItem[]> = {
...
declare type Context<A extends IdlAccountItem[]> = Omit<AnchorContext, "accounts"> & {
    accounts: ContextAccounts<A>;
};
declare type MakeInstructionsNamespace<R extends InstructionsParsed, Ret, Mk extends {
...
declare type RpcNamespace<R extends InstructionsParsed> = MakeInstructionsNamespace<R, Promise<TransactionSignature>>;
declare type InstructionNamespace<R extends InstructionsParsed> = MakeInstructionsNamespace<R, TransactionInstruction, {
    [M in keyof R]: {
...
declare type DecodeType<T extends IdlType, Defined> = T extends keyof TypeMap ? TypeMap[T] : T extends {
...
declare type MakeArgs<A extends IdlField[], Defined> = {
    [K in keyof A]: A[K] extends IdlField ? DecodeType<A[K]["type"], Defined> : unknown;
};
declare type MakeInstructions<I extends IdlInstruction[], Defined> = {
    [K in I[number]["name"]]: {
        accounts: (I[number] & {
            name: K;
        })["accounts"];
...
export declare type AnchorProgram<IDL extends Idl, A, Defined = AnchorDefined<IDL>, RPCInstructions extends MakeInstructions<IDL["instructions"], Defined> = MakeInstructions<IDL["instructions"], Defined>, Methods extends MakeInstructions<NonNullable<IDL["state"]>["methods"], Defined> = MakeInstructions<NonNullable<IDL["state"]>["methods"], Defined>> = Omit<AProgram, "rpc" | "state" | "account" | "transaction" | "instruction"> & {
...
declare type ErrorMap<T extends Idl> = {
...
declare type AnchorEvent<T extends IdlEventField, Defined> = {
    [N in T["name"]]: DecodeType<(T & {
        name: N;
    })["type"], Defined>;
};
declare type AnchorEvents<T extends IdlEvent, Defined> = {
    [K in T["name"]]: {
        name: K;
        data: AnchorEvent<(T & {
            name: K;
